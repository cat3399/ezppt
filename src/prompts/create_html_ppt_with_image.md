...
8. 水平时间轴/路线图布局 (Horizontal Timeline/Roadmap Layout)
   *   **布局说明**：水平中心线上，要点分布在一侧，形成路线图。
   *   **核心特点**：韵律感、叙事性。
   *   **最佳适用**：发展历程、计划路线图。
...
- **画布与背景 (Canvas & Backdrop)**
  - **画布 `#canvas`**: 所有内容必须包裹于此。CSS 必须为 `width: 1280px; height: 720px;` 且无任何 `margin`。
  - **背景 `<body>`**: 必须用于居中画布，样式为 `display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0;`
  - **背景设定规则 (Background Setting Rules)**:
    - **`target_id` 为 1.1 (开篇页特殊规则)**: 如果 `imgs_info` 中有合适的图片，你**可以**选择一张作为全局背景。**必须**在图片上叠加一层半透明的纯色蒙版以确保文字的可读性。如果提供的图片不适合作为背景（如过于杂乱或尺寸不符），则应退而使用纯色背景。
    - **所有其他页面 (target_id 非 1.1)**: **必须**严格继承 `style_reference_html` 的背景设置。**严禁**使用 `imgs_info` 中的图片作为这些页面的全局背景。
...
- **技术栈与格式 (Tech Stack & Format)**
  ...
  - **图片使用规范 (Image Usage Specification)**
    - **图片来源**: **只能**使用 `imgs_info` 中提供的图片。通过 `<img>` 标签引用，`src` 属性应为提供的本地路径。
    - **构图**: 根据图片的分辨率安排合适的布局,同时可以使用CSS对图片进行放大缩小或者适当的拉伸(拉伸比例不要太严重)以便于适应布局。一般不建议裁剪图片。
    - **图片与文字关系**: 如果图片作为局部背景，必须确保前景文字有足够的对比度。
    - **图片角色 (Image Role)**: 图片主要用作构图中的**视觉元素**（如卡片内容、视觉锚点），而非背景。唯一的例外是 `target_id` 为 1.1 的页面，它**可以**（非必须）根据**背景设定规则**使用图片作为全局背景。
    - **风格一致性**: 图片本身不应添加阴影、发光等特效。可以为其容器添加与整体风格一致的边框（`border`）或圆角（`border-radius`）。
