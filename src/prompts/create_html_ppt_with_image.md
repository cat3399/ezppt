...
- **提供的图片 (imgs_info)**: `<<{imgs_info}>>` 
  - **说明**: 这是一个包含图片信息（本地路径和描述）的列表。**图片内容不一定与幻灯片文本直接相关**。你必须将它们视为**构图元素**或**氛围营造工具**，而非简单的内容插图。**如果提供了图片，你必须在设计中使用它，并确保其在布局中扮演有意义的角色。**
...
- **技术栈与格式 (Tech Stack & Format)**
  ...
  - **图片使用规范 (Image Usage Specification)**
    ...
    - **构图**: 可以使用CSS对图片进行放大缩小或者适当的拉伸(拉伸比例不要太严重)以便于适应画布。一般不建议裁剪图片
    - **图片与文字关系**: 如果图片作为局部背景，必须确保前景文字有足够的对比度，可以考虑在图片上叠加一层半透明的纯色蒙版（如 `rgba(0,0,0,0.4)`）来压暗背景，突出文字。除非你负责target_id为1.1的页面，否则禁止使用imgs_info中的图片作为全局背景。
...
3. **背景图片 (Background Image)**：如果 `style_reference_html` 使用了本地路径的图片，你也可以将其作为背景图片使用。
...
2.  **图像整合策略 (Image Integration Strategy)** 
    ...
    -   **决定图片角色**: 基于 `Layout Directive` 和图片特征，决定图片在构图中的角色：
        ...
        -   **氛围背景**: 图片可作为整个画布或特定区域（如卡片）的背景，为内容提供情境或情绪基调。
...
- **背景**：你必须按照从 `style_reference_html` 提取的色彩体系来设置`canvas`的背景色。并且保持背景的纯净,不得在整个画布的背景上添加任何装饰性元素（除非图片被用作背景）。
- **图标**：
