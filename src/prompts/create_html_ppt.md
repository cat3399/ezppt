...
#### 阶段二：指令驱动的布局执行 (Directive-Driven Layout Execution)

这是你工作的核心。你的任务不再是自由创作，而是**精确地、创造性地执行**来自 `Layout Directive` 的宏观布局指令。

1.  **解码指令 (Decode the Directive):** 仔细阅读 `Layout Directive` 的文字描述。理解其核心要求：是分栏、居中、网格还是其他结构？视觉重心在哪里？
2.  **内容映射 (Map Content to Structure):** 分析 `target_id` 的具体内容（标题、要点数量、文本长度），并将其**映射**到指令所描述的布局结构中。例如，如果指令是“双栏布局”，你需要决定哪些内容放在左栏，哪些放在右栏。
3. **灵活调整 (Flexible Adjustment):** 在满足Layout Directive的布局前提下,可以自由调整元素的间距、大小、颜色等，以达到最佳的视觉效果。
4.  **细节微调与连贯性 (Refine with Continuity):**
    *   在实现了宏观布局后，如果提供了 `continuity_reference_html`，请观察其中的**微观设计元素**，如卡片的 `border-radius`、元素间的 `gap`、标题与正文的垂直间距等。
    *   将这些细节应用到你当前的设计中，以确保**同一章节内**的页面在视觉上是**和谐统一**的，即使它们的宏观布局不同。**这是一种细节上的继承，而非结构上的复制。**
