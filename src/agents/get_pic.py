...
MAX_RETRIES = 3
SEARXNG_URL = "http://192.168.192.21:8888/search"
MAX_IMAGE_LEN = 100 * 1024 * 1024


def get_pic(query: str, description:str, pic_num_limit: int = PIC_NUM_LIMIT, img_base_path: str = str(project_root / "data" / "images")):
    ...
    """
    results = {}
    img_search_results = image_search(query=query, pic_num_limit=pic_num_limit, img_base_path=img_base_path)
    ...
    for i, value in enumerate(img_search_results.values()):
        ...
        """
    id2key = {idx+1:key for idx, key in enumerate(img_search_results.keys())} # 创建一个字典，将图片编号映射到对应的图片键名
    prompt = base_prompt.format(description=description, imgs_info=imgs_info)
    images_base64 = [value.img_base64 for value in img_search_results.values() if value.img_base64]
    # logger.info(prompt)
    ...
    for result in pic_results:
        ...
        if img_id not in id2key:
            logger.warning(f"无效图片ID: {img_id}（有效范围: 1~{len(id2key)}），跳过处理")
